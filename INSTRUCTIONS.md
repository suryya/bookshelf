...
1. To login, you need to send a `POST` request to the `login` endpoint with the
   `JSON` body `{"username": "SomeUsername", "password": "SomePassword"}` (that
   requires that you set the `Content-Type` header to `application/json`). This
   will respond with a `JSON` object that has a user property with a `token`
   which you should store in `localStorage` and the rest of the user's data.
...
```javascript
client('http://example.com/pets', {data: {name: 'Fluffy', type: 'cat'}})
...
//    - Authorization: 'Bearer AUTH_TOKEN' // assuming there is a token
```
