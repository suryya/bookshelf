...
to the level of confidence we can acheive relative to the amount of work they
take.
...
means you'll need to "log in" before rendering the component (which just means
you'll need a token in local storage). In addition, because we want to test at
as high a level as possible, we'll be rendering the `App` component with all the
`AppProviders`. We'll also want to mock `window.fetch` so we can respond with
fake data, and then we can instruct our test to wait for react-query to finish
fetching all the data and for the loading indicators to go away. Once that's all
done, then we can start interacting with the page.
...
I'll bet you can figure out how to do that 😉
...
> explicitely. Handle that in your implementation.
...
For the second, you'll need to mock out `window.fetch` directly so it responds
with an error. Try:
